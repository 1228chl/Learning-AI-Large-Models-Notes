...
原子卡片是基于原子化知识理念设计的知识点卡片，每张卡片围绕一个独立概念展开，包含定义（含 LaTeX 公式）、核心公式、直观理解、ML/DL 应用场景、面试追问 Q1-Q4 以及跨域引用链接。共 **251 张卡片**，覆盖 14 个分类。

| 分类 | 子分类 | 卡片数 | 说明 |
|:-----|:-------|:------:|:-----|
| 线性代数 | 向量 / 矩阵 / 特征分解 / 应用 | 18 | 向量空间、矩阵运算、特征值、正交投影等 |
| 微积分与优化 | -- | 6 | 导数、链式法则、梯度下降、凸优化 |
| 概率统计 | -- | 9 | 概率空间、贝叶斯公式、协方差、贝叶斯推断 |
...
| Python | OOP / Pydantic / 并发 / 工具 | 19 | 类与对象、Pydantic 数据建模、asyncio 异步、装饰器等 |
| Linux | -- | 8 | 基础命令、文件管理、进程、网络监控 |
| 数据库 | SQL / Redis / Milvus / 检索 | 21 | SQL 基础、Redis 数据结构、向量数据库、嵌入与检索 |
| 数据结构与算法 | 基础结构 / 树堆图 / 算法 | 15 | 数组、链表、栈、树、图、排序、动态规划 |
| AI-Agent | 基础 / LangChain / LangGraph / RAG 流程 / 检索 / 系统 / 设计模式 / 工程实践 / 基础设施 | 36 | Agent 定义、RAG 实现、多 Agent 系统、BM25、Coze、CoT 等 |
| 数据分析 | -- | 4 | NumPy、Pandas、Matplotlib |
| 工程实践 | Docker / 部署 / 工具 / 网络 | 17 | Docker 编排、Git、LLM API 部署、Vue3 前端集成、SSE 流式输出等 |
| 知识体系 | -- | 2 | 核心依赖链、面试追问树 |
...
| 路径 | 说明 |
|:-----|:------|
| **Atomic-Cards/** | 原子卡片库（251 张，14 分类） |
