...
Lasso（L1）在特征高度相关时的问题：假设两个特征 $x_1$ 和 $x_2$ 高度相关，Lasso 会随机选择其中一个，将另一个的权重压为零——这导致模型不稳定，换一组数据可能选到另一个特征。

ElasticNet 的解决方案：在 L1 惩罚的基础上添加 L2 惩罚。L2 的圆形约束倾向于同时保留相关特征（因为 $\|w_1\|^2 + \|w_2\|^2$ 在 $w_1 \approx w_2$ 时比 $w_1 \gg 0, w_2 = 0$ 更小），这被称为"群组效应"——相关特征组被一起选入或一起排除，而不是随机选择其中一个。

### 选择策略

- 特征数量少且相关性强 → ElasticNet
- 只关心稀疏性 → Lasso
- 所有特征都有用 → Ridge
...
- 需要了解正则化总览以理解L1与L2的正则化框架，参见 [正则化](01-正则化.md)
