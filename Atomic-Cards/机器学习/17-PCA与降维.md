...
**回答要点**：PCA 是线性降维方法，通过最大化方差方向投影，保留全局数据结构，适合加速训练、去噪、去相关；t-SNE 是非线性降维，通过概率分布匹配保留局部邻域关系，仅适合可视化探索。PCA 可逆（有解析变换矩阵），t-SNE 不可逆且每次运行结果不同。

**Q2（深挖）**：PCA 中"解释方差比"（explained variance ratio）的含义是什么？如何根据它选择主成分数量？

**回答要点**：解释方差比 = 该主成分对应特征值 / 所有特征值之和，表示该方向保留的原始方差比例。选择主成分数量的常用方法：累计方差比达到阈值（如 95%）、特征值 > 1（Kaiser 准则）、或通过 Scree Plot 找"肘部"拐点。

**Q3（实战）**：在使用 PCA 处理真实数据时，是否需要对特征进行标准化/归一化？为什么？

**回答要点**：必须标准化。PCA 寻找方差最大的方向，如果特征量纲不同（如一个特征范围 0-1，另一个 0-10000），方差会被量纲大的特征主导，主成分方向扭曲。正确做法：先做 StandardScaler（均值为 0，方差为 1）再进行 PCA。

**Q4（边界）**：PCA 的线性假设在什么情况下会失效？如何判断数据是否需要非线性降维？

**回答要点**：数据位于非线性流形上（如瑞士卷、圆环分布）时，PCA 无法有效展开结构——投影后的低维表示会混叠原始类别。判断方法：PCA 累计方差需要很多维度才能达到阈值（线性假设不成立）、可视化发现降维后类别重叠严重。此时应使用 t-SNE、UMAP 或自编码器。
