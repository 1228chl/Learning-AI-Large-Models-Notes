...
**回答要点**：监督学习有标签，每个样本都有反馈，目标是拟合 X→y 映射；无监督学习无标签，目标是发现隐藏结构；强化学习只有延迟奖励信号，通过试错学习最优策略；三者反馈方式和目标不同。

**Q2（深挖）**：分类任务和回归任务有什么区别？各自用什么损失函数和评估指标？

**回答要点**：分类输出离散类别，回归输出连续数值；分类常用交叉熵损失（分类）或 Hinge 损失，评估用准确率/F1/AUC；回归常用 MSE 损失，评估用 R²/RMSE/MAE；选错会导致模型训练不收敛或评估误导。

**Q3（实战）**：在一个新项目初期，如何判断应该用哪种学习范式？

**回答要点**：先看是否有标签：有标签→监督学习，无标签→无监督学习；再看数据类型和目标：预测离散值还是连续值（分类/回归）；若涉及序列决策和交互环境→强化学习；业务可接受成本决定是否标注数据。

**Q4（边界）**：监督学习在什么场景下会失效？有什么补救措施？

**回答要点**：标注成本过高或无法获取标签时失效；补救措施包括半监督学习（少量标签+大量无标签）、主动学习（只标注最有价值的样本）、弱监督（利用启发式规则生成噪声标签）。

> 理解前置知识可参见 [机器学习概述](./01-机器学习概述.md)；理解前置知识可参见 [过拟合与欠拟合](./05-过拟合与欠拟合.md)；理解前置知识可参见 [PCA与降维](./17-PCA与降维.md)