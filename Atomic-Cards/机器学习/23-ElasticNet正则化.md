...
**回答要点**：L1 项产生稀疏解做特征选择，L2 项使相关特征的权重趋向于相等（群组效应），两者结合弥补了 Lasso 在相关特征中随机选择的不足；核心优势是高维 + 特征高度相关时，既能降维又能稳定保留相关特征群组。

**Q2（深挖）**：ElasticNet 中 l1_ratio 参数从 0 到 1 变化时，模型行为如何变化？

**回答要点**：l1_ratio=0 时为纯 Ridge（所有权重缩小但不为零）；l1_ratio=1 时为纯 Lasso（稀疏，相关特征随机选）；在 0~1 之间时 L1 控制稀疏程度，L2 控制群组效应强度；l1_ratio 较小时（接近 0）行为类似 Ridge 但仍有稀疏性，较大时（接近 1）行为类似 Lasso 但有群组效应。

**Q3（实战）**：ElasticNetCV 的两个超参数 alpha 和 l1_ratio 如何同时调优？实践中怎么做？

**回答要点**：ElasticNetCV 支持在 alpha 和 l1_ratio 的网格上联合交叉验证；实践中先在对数尺度设 alpha 网格（如 [1e-4, 1e-3, 0.01, 0.1, 1]），l1_ratio 在 [0.1, 0.3, 0.5, 0.7, 0.9] 上搜索；计算量较大（网格乘积），大数据集上可先用随机搜索或粗网格再局部精调。

**Q4（边界）**：ElasticNet 在什么情况下不如纯 Lasso 或纯 Ridge？

**回答要点**：当特征间确实不存在相关结构时，ElasticNet 的 L2 项引入不必要的群组效应，增加模型复杂度；数据量小且特征关系清楚时，选择更明确的纯 Lasso 或 Ridge 更优；ElasticNet 需要同时调 alpha 和 l1_ratio 两个超参数，调参空间是 Lasso/Ridge 的两倍，计算成本更高。

## 参考引用

- 需要了解正则化总览以理解L1与L2的正则化框架，参见 [正则化](./20-正则化.md)
- 需要掌握L2正则化的权重衰减机制以对比ElasticNet，参见 [L2正则化(Ridge)](./21-L2正则化(Ridge).md)
- 需要掌握L1正则化的稀疏解特性以理解ElasticNet的特征选择，参见 [L1正则化(Lasso)](./22-L1正则化(Lasso).md)