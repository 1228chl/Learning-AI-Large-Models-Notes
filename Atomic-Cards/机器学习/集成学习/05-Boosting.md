...
Boosting 与 Bagging 的几何直觉截然不同：

- **Bagging**：多个独立判断者投票，错误相互抵消
- **Boosting**：一个接一个地修正前一个人的错误，每次修正当前最大的错误

AdaBoost 通过调整样本权重实现：上一轮被分错的样本权重增加，下一轮模型必须更关注这些"硬骨头"。梯度提升（GBDT）则更直接：每轮拟合前一轮预测的**残差**（即错误的方向和大小）。

### 梯度提升的函数空间优化

将梯度提升理解为**在函数空间中的梯度下降**：$F(x)$ 是我们要优化的函数，每轮迭代找到使损失下降最快的"函数方向"（即负梯度方向），然后沿着这个方向迈出一步 $\nu \cdot \gamma_m h_m(x)$。每棵树 $h_m(x)$ 就是对这个函数方向的一个近似——它拟合的不是原始数据，而是当前损失函数的负梯度。

这种视角统一了梯度提升与神经网络的梯度下降：前者在函数空间迭代，后者在参数空间迭代。
...
其中 $w_i^{(t)}$ 为第 $t$ 轮中第 $i$ 个样本的权重，$w_i^{(t+1)}$ 为更新后的权重；$\alpha_t$ 是第 $t$ 个弱学习器的权重，$h_t(x_i)$ 为其预测结果，$y_i \in \{-1, +1\}$ 为真实标签。
...
其中 $F_m(x)$ 为第 $m$ 步的集成模型，$F_{m-1}(x)$ 为前 $m-1$ 步的集成模型；$\nu$ 为学习率，$\gamma_m$ 为步长，$h_m(x)$ 拟合前一轮的负梯度 $-\nabla_{F} L(y, F(x))$ 。
...
| 应用场景           | 数学形式                                                                              | 说明                                            |
| -------------- | --------------------------------------------------------------------------------- | --------------------------------------------- |
| GBDT / XGBoost | $F_m(x) = F_{m-1}(x) + \eta \cdot \arg\min_h \sum_i L(y_i, F_{m-1}(x_i) + h(x_i))$ | 竞赛和工业界最常用的表格数据建模方法（$\eta$ 为学习率，$L$ 为损失函数）                            |
| AdaBoost 人脸检测  | $H(x) = \sum_{t=1}^T \alpha_t h_t(x)$                                             | Viola-Jones 人脸检测器：级联弱分类器实现实时检测。$H(x)$ 为最终强分类器，$T$ 为弱分类器总数                |
| CatBoost       | 对称决策树 + 有序提升                                                                      | 处理类别特征的专用梯度提升变体                               |
| 深度 Boosting    | 残差网络（ResNet）                                                                      | $x_{l+1} = x_l + F(x_l)$ 可视为 Boosting 思想的深度实现。其中 $x_l$ 为第 $l$ 层的输出，$x_{l+1}$ 为下一层的输出，$F(x_l)$ 为残差映射函数 |
