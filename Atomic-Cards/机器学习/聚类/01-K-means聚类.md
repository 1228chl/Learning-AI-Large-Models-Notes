...
### K-means 的 Voronoi 划分原理

K-means 的几何直觉：给定 $K$ 个簇中心，K-means 将特征空间划分为 $K$ 个**Voronoi 区域**——每个区域包含所有离对应中心最近的样本点。算法通过"分配-更新"的交替迭代逐步优化这些中心的位置：

1. **分配**：每个样本归到最近的簇中心 → 等价于用簇中心对空间做 Voronoi 划分
2. **更新**：将簇中心移到簇内点的均值位置 → 等价于找到该区域中使惯性最小的"代表点"

这两个步骤交替进行，簇中心逐渐移动到数据密集区域的中心。初始中心的位置（随机或 K-means++）决定了最终收敛到哪个局部最优。

### 球状簇假设的几何含义

K-means 使用欧氏距离，这隐含假设了各簇是**球状且各向同性**的。几何上，每个簇的等距面是圆形（二维）或球形（高维）。当真实簇的形状偏离球状时（如月牙形、长条形），K-means 的硬性 Voronoi 划分会将一个不规则簇切成几块，或将多个簇合并——这是其最根本的局限性。
...
## 参考引用
- 需要理解层次聚类与DBSCAN的相关知识，参见 [层次聚类与DBSCAN](02-层次聚类与DBSCAN.md)
