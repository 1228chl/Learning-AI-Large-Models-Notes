...
1. 容器共享宿主机内核（轻量、秒级启动），VM 包含完整 Guest OS（隔离性强、资源占用大）
2. 容器隔离依赖 cgroups/namespace，安全性弱于 VM
3. 容器适合微服务和 CI/CD，VM 适合强隔离需求

**Q3（实战）**：在 ML 项目中使用 Docker 进行模型部署时，如何处理 GPU 支持和多服务编排？
**回答要点**：

1. 使用 `nvidia/cuda` 基础镜像并安装 `nvidia-container-toolkit`
2. 通过 `docker compose` 编排 API 服务、向量数据库、缓存等多容器
3. 注意 CUDA 版本与 PyTorch/TensorFlow 版本的匹配

**Q4（边界）**：Docker 在 ML 场景中有哪些局限性？何时应选择其他方案？
**回答要点**：

1. Windows 下 GPU 支持不完善（需 WSL2）
2. 分布式训练的多节点通信配置复杂
3. 大模型镜像体积过大（可用多阶段构建优化）
4. 高性能计算场景考虑 Singularity/Apptainer

## 参考引用
- 需要理解 Ollama 与本地 LLM 部署的相关知识，参见 [Ollama与本地LLM部署](../部署/06-Ollama与本地LLM部署.md)
- 需要理解 Flask 与 FastAPI 模型部署的相关知识，参见 [Flask与FastAPI模型部署](../部署/04-Flask与FastAPI模型部署.md)
