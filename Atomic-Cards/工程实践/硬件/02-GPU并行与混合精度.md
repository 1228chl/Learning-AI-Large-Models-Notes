...
1. 同时使用 FP16 进行前向和反向传播以节省显存和加速计算，使用 FP32 进行权重更新以保持精度
2. 通过 GradScaler 在反向传播前放大 loss，防止 FP16 梯度下溢为 0，更新权重前再缩小复原
3. 几乎不损失精度的情况下将训练速度提升 2-3x，显存占用减半

**Q2（深挖）**：FP16 和 BF16 有何区别？为什么 BF16 在训练中越来越受欢迎？
**回答要点**：

1. FP16 有 5 位指数 + 10 位尾数，动态范围窄（约 5.96e-8 ~ 65504），训练中容易出现溢出问题
2. BF16 有 8 位指数 + 7 位尾数，动态范围与 FP32 完全相同（约 1.18e-38 ~ 3.4e38），仅尾数精度略低
3. BF16 不需要 loss scaling，训练更稳定，因此在大模型训练中越来越受欢迎

**Q3（实战）**：你在项目中使用 `torch.cuda.amp` 遇到过哪些坑？如何排查显存不足问题？
**回答要点**：

1. 容易遗忘调用 `scaler.update()` 导致缩放因子无法动态调整，影响训练稳定性
2. 某些算子（如 LayerNorm、Softmax）不支持 AMP，需手动强制 FP32 执行以避免精度损失
3. 显存不足时优先使用梯度检查点（gradient checkpoint）、梯度累积减少峰值显存，或逐步减半 batch_size 排查

**Q4（边界）**：分布式训练中数据并行（DDP）和模型并行各自的最佳适用场景是什么？混合并行策略如何选择？
**回答要点**：

1. DDP 适用于模型能装入单卡 GPU 的场景，每个进程维护完整模型副本，通信开销为梯度同步，PyTorch 官方推荐使用
2. 模型并行（含张量并行和流水线并行）适用于单卡放不下的超大模型，通信开销为中间激活传输，但可突破单卡显存上限
3. 超大 Transformer 模型常组合数据并行 + 张量并行 + 流水线并行，在规模、通信效率和计算利用率之间取得平衡

## 参考引用
- 需要理解PyTorch张量与运算的相关知识，参见 [PyTorch张量与运算](../../深度学习/PyTorch/07-PyTorch张量与运算.md)
- 需要掌握进程与线程以理解编程实现机制的相关知识，参见 [进程与线程](../../Python/并发/06-进程与线程.md)
- 需要理解PyTorch张量与运算的深度学习机制与实现的相关知识，参见 [PyTorch张量与运算](../../深度学习/PyTorch/07-PyTorch张量与运算.md)
