...
> 理解前置知识可参见 [Docker基础与容器化](./01-Docker基础与容器化.md)；理解前置知识可参见 [GPU并行与混合精度](./02-GPU并行与混合精度.md)；理解前置知识可参见 [HTTP基础与API设计](./08-HTTP基础与API设计.md)；理解数据集划分与交叉验证的机器学习原理可参见 [数据集划分与交叉验证](../机器学习/03-数据集划分与交叉验证.md)