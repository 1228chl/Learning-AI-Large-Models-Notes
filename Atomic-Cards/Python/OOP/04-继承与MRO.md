...
当继承导致 MRO 复杂化时，优先考虑**组合**：将功能委托给独立类，而非通过继承获得。

| 方式 | 关系 | 耦合度 | 灵活性 |
|:----|:----|:------:|:------:|
| **继承** | is-a | 高（子类依赖父类实现） | 低 |
| **组合** | has-a | 低（只依赖接口） | 高 |
| **Mixin** | 混合 | 中 | 中 |

**经验法则**：能用组合解决的问题，不要用继承。当继承深度超过 3 层或出现菱形继承时，应优先考虑重构为组合或 Mixin。
...
- 需要理解多态与鸭子类型的相关知识，参见 [多态与鸭子类型](05-多态与鸭子类型.md)
