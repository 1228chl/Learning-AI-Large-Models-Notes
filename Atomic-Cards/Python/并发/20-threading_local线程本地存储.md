...
- 需要理解 Python 多线程与 GIL 的基本机制：[线程与 GIL](./15-线程与GIL.md)
- 需要理解 Python 上下文管理器协议（与 threading.local 配合清理资源）：[上下文管理器](../工具/05-上下文管理器.md)
- 需要理解协程（asyncio）环境下 contextvars 的使用（与 threading.local 的替代关系）：[协程与 asyncio](./10-协程与asyncio.md)
- 需要理解 SQLAlchemy 异步会话管理（与线程本地连接的相似性）：[SQLAlchemy 异步操作三件套](../../数据库/SQL/06-SQLAlchemy异步操作三件套.md)
- 需要理解后台任务 GC 保护中 `_background_tasks` 集合防止对象被回收的机制：[后台任务 GC 保护模式](./18-后台任务GC保护模式.md)
