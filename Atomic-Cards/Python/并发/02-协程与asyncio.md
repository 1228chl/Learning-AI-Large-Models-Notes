...
1. 调度方式：协程由程序自身协作式调度，线程由操作系统抢占式调度
2. 切换开销与内存占用：协程切换在用户态（纳秒级），内存约 1KB/个；线程切换需内核态（微秒级），内存约 8MB/个
3. 并发与锁：协程单线程无需加锁，线程需处理锁竞争和同步问题

**Q2（深挖）**：`await` 关键字的作用是什么？它和 Python 的生成器 `yield` 在机制上有哪些异同？
**回答要点**：

1. `await` 挂起当前协程，将控制权交还给事件循环，等待另一个 awaitable 对象完成后恢复执行
2. `async/await` 底层基于生成器实现（PEP 492），`await` 类似 `yield from` 但不完全相同
3. `await` 语义上用于等待另一协程完成并获取结果，`yield` 用于生成值或实现双向通信

**Q3（实战）**：在 FastAPI 中，异步接口处理 I/O 请求时为什么能提升吞吐量？所有视图函数都应该声明为 `async` 吗？
**回答要点**：

1. 异步接口在处理 I/O 等待时通过 `await` 释放线程给其他请求，避免线程池耗尽
2. 并非所有函数都应声明为 `async`，纯 CPU 计算用 `async` 无实际收益且增加调度开销
3. 推荐在调用异步库（数据库、HTTP、文件 I/O）时用 `async`，纯计算用同步函数

**Q4（边界）**：`asyncio.gather` 并发执行多个协程时，如果其中一个抛出异常会怎样？如何控制并发数量防止资源耗尽？
**回答要点**：

1. 默认情况下任一协程抛出异常会立即传播到调用方，其他协程仍执行但结果不会被收集
2. 可使用 `return_exceptions=True` 将异常作为结果返回，由调用方统一处理
3. 使用 `asyncio.Semaphore` 限制同时运行的任务量，防止大量并发请求压垮下游服务或触发连接池耗尽

## 参考引用
- 需要理解进程与线程的相关知识，参见 [进程与线程](01-进程与线程.md)
- 需要理解迭代器与生成器的相关知识，参见 [迭代器与生成器](../工具/02-迭代器与生成器.md)
- 需要理解Socket网络编程的相关知识，参见 [Socket网络编程](../工具/06-Socket网络编程.md)
