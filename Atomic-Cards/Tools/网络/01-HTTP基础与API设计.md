...
HTTP 协议本身不保存客户端状态——每个请求都是独立的，服务器不"记住"前一个请求。这意味着：
- 每次 LLM API 调用都需要完整传入对话历史（messages 数组），服务器不维护会话
- 多轮对话的"记忆"由客户端（应用层）负责拼接，而非 HTTP 协议

### 流式输出的协议选择

LLM 流式对话中，HTTP/1.1 SSE 是单向流（仅服务端→客户端）。当需要双向实时通信时：

| 方案 | 方向 | 适用场景 |
|:----|:----|:---------|
| **SSE（HTTP/1.1）** | 服务端→客户端 | LLM 逐 token 输出 |
| **WebSocket** | 全双工 | 实时交互对话 |
| **gRPC（HTTP/2）** | 双向流 + 强类型 | 微服务间高性能通信 |
...
## 参考引用
- 需要理解Flask与FastAPI模型部署...的相关知识，参见 [Flask与FastAPI模型部署](../部署/02-Flask与FastAPI模型部署.md)
- 需要理解LLM API调用与ChatBot...的相关知识，参见 [LLM API调用与ChatBot](../部署/05-LLM API调用与ChatBot.md)
- 需要掌握Socket网络编程以理解编程实现机制...的相关知识，参见 [Socket网络编程](../../Python/工具/06-Socket网络编程.md)
