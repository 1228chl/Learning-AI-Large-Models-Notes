...
过度剪枝导致模型容量不足以拟合数据（欠拟合），被剪枝的参数可能恰好包含重要信息导致精度骤降。缓解策略：**迭代式剪枝**（逐步增加稀疏度，每步微调）、**学习率回退**（剪枝后用小学习率恢复）、结合知识蒸馏（以大模型为教师指导学生剪枝后模型）。
...
1. 非结构化剪枝以单个权重为粒度，产生稀疏矩阵，通用 GPU 无法直接加速
2. 结构化剪枝以通道或层为粒度，移除后仍保持密集矩阵乘法
3. 结构化剪枝可在通用 GPU 上直接获得推理加速，非结构化需专用稀疏张量核心

**Q2（深挖）**：为什么非结构化剪枝在通用 GPU 上难以获得实际加速？
**回答要点**：

1. 非结构化剪枝产生稀疏权重矩阵，GPU 并行计算基于密集矩阵乘法优化
2. 稀疏矩阵需特殊硬件或库支持（如 NVIDIA 稀疏张量核心），通用 GPU 无法直接利用
3. 结构化剪枝移除后权重仍为密集矩阵，因此可直接获得加速

**Q3（实战）**：如何评估一个权重或通道的重要性？
**回答要点**：

1. 基于权重大小（L1/L2 范数），绝对值越小越不重要
2. 基于梯度（Fisher 信息量），梯度小的参数所含信息量少
3. 基于特征图响应（BN 层缩放因子 $\gamma$），$\gamma$ 接近 0 的通道可移除
4. 基于二阶信息（OBD/OBS 使用 Hessian 矩阵）

**Q4（边界）**：剪枝后模型可能出现什么副作用？如何缓解？
**回答要点**：

1. 过度剪枝导致欠拟合，模型容量不足以拟合数据
2. 被剪枝参数可能包含重要信息，造成精度骤降
...
- 需要了解模型压缩总览的相关知识，参见 [模型压缩量化剪枝蒸馏](02-模型压缩量化剪枝蒸馏.md)
