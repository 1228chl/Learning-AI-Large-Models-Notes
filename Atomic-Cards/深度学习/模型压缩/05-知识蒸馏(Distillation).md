...
当教师模型过大而学生能力不足时，学生无法拟合教师的复杂决策边界——称为**容量差距**。教师自身的偏见和错误也会被蒸馏传递给学生，造成错误放大。应对策略：使用中间层特征蒸馏（FitNet 让学生学习教师中间层的表示）或渐进式蒸馏（先蒸馏到中等模型，再二次蒸馏到小模型）。
...
- 需要理解模型压缩量化剪枝蒸馏的相关知识，参见 [模型压缩量化剪枝蒸馏](02-模型压缩量化剪枝蒸馏.md)
- 需要理解知识蒸馏详解的相关知识，参见 [知识蒸馏详解](01-知识蒸馏详解.md)
