...
- 需要理解卷积运算的基本原理参见 [卷积运算](./09-卷积运算.md)
- 需要理解池化层的下采样作用参见 [池化层](./10-池化层.md)
- 需要理解残差连接的设计原理参见 [残差连接与LayerNorm](../../NLP/组件/09-残差连接与LayerNorm.md)
- 需要理解迁移学习中 backbone 的使用参见 [迁移学习与微调](../迁移学习/17-迁移学习与微调.md)
- 需要理解 GPU 训练大规模 CNN 的并行策略参见 [GPU并行与混合精度训练](../训练优化/02-GPU并行与混合精度.md)