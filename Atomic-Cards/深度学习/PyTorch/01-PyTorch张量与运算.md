...
| **模型参数** | 各层 $W$ 形状不同 | $W$ 为权重矩阵；线性层 $(d_{in}, d_{out})$，$d_{in}$ 为输入维度、$d_{out}$ 为输出维度 |
| **梯度** | 与对应参数形状相同 | $\frac{\partial L}{\partial W} \in \mathbb{R}^{d_{in} \times d_{out}}$（$L$ 为损失函数） |
...
1. PyTorch Tensor 支持 GPU 加速（.cuda()），NumPy 仅 CPU
2. Tensor 集成自动微分（requires_grad），NumPy 无此功能
3. 两者可互相转换（torch.from_numpy() / .numpy()），共享内存需注意同步

**Q2（深挖）**：x.view() 和 x.reshape() 有什么区别？
**回答要点**：

1. view() 要求张量在内存中连续（contiguous），否则需先调用 .contiguous()
2. reshape() 自动处理连续性，在不连续时返回副本而非视图
3. 用 view() 更高效（无拷贝），但在 Transpose 等操作后需注意连续性

**Q3（实战）**：训练时遇到 "CUDA out of memory"，你会从哪些方面优化？
**回答要点**：

1. 减小 batch size，这是最直接的显存优化手段
2. 用 torch.no_grad() 禁用推理时的梯度计算
3. 使用梯度累积模拟大 batch，并开启混合精度训练（AMP）减少显存占用

**Q4（边界）**：张量广播机制在什么情况下会导致隐式 bug 或性能问题？
**回答要点**：

1. 广播在维度不匹配时自动扩展，可能导致意外的大张量创建
2. 大维度广播显著增加显存和计算开销
3. 调试时用 .shape 显式检查，避免隐式广播掩盖形状不匹配的错误

## 参考引用
- 需要理解自动微分机制的相关知识，参见 [自动微分机制](02-自动微分机制.md)
- 需要理解损失函数的相关知识，参见 [损失函数](../基础/03-损失函数.md)
