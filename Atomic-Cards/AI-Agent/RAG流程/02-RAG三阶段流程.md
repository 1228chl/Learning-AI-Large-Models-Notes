...
1. 索引阶段：文档切分→向量化→存入向量库，解决文档如何被结构化存储以便检索的问题
2. 检索阶段：查询向量化→向量库 Top-K 搜索，解决如何找到与查询最相关文档的问题
3. 生成阶段：检索结果+原始查询→LLM 生成，解决如何基于事实而非模型参数记忆生成答案的问题

**Q2（深挖）**：RAG 相比模型微调在知识更新和幻觉控制上各有什么优劣？各自的最佳场景是什么？
**回答要点**：

1. 知识更新：RAG 修改知识库即可实时更新，微调需重新训练完整流程，成本高、周期长
2. 幻觉控制：RAG 通过检索结果约束 LLM 输出，可有效减少幻觉但受检索引擎上限影响；微调仍可能产生幻觉但输出格式和风格一致性更好
3. 最佳场景：RAG 适合知识密集型问答（如企业知识库），微调适合输出格式/风格迁移（如特定回复模板）

**Q3（实战）**：当用户查询比较简短模糊时（如"什么是注意力机制？"），你会采用什么检索策略提升效果？
**回答要点**：

1. HyDE 策略：先让 LLM 基于短查询生成一段假设的详细答案，再用假设答案做向量检索，补充语义信息使检索更准确
2. 子查询策略：将模糊查询拆分为多个具体子问题（如"注意力机制的定义"、"注意力机制的应用"）分别检索再合并去重
3. 查询改写：先对用户原始查询进行意图识别和改写，补充关键语义后再执行检索

**Q4（边界）**：RAG 在什么场景下检索效果显著下降？如何系统性地应对？
**回答要点**：

1. 高度专业术语且知识库覆盖不足时：补充领域文档或结合 BM25 精确匹配，提升检索召回率
2. 需要多跳推理的复杂问题：采用回溯检索（RQ-RAG）让模型根据已检索到的内容迭代构造新查询，逐步逼近答案
3. 知识库质量差、噪音多：引入重排序（Reranker）过滤低质量结果，并优化 Chunk 切分策略（调整 chunk_size 和 overlap）
...
- 需要了解Milvus核心概念以理解数据存储与检索技术，参见 [Milvus核心概念](../../数据库/Milvus/08-Milvus核心概念.md)
- 需要理解RAG查询改写与意图识别的相关知识，参见 [RAG查询改写与意图识别](../系统/27-RAG查询改写与意图识别.md)
