...
**回答要点**：残差连接将输入直接加到输出，梯度反向传播时通过恒等映射形成"梯度高速公路"，梯度可以直接从深层流向浅层而不经过权重矩阵相乘，避免了链式法则中梯度逐层衰减的问题。

**Q2（深挖）**：Pre-LN 和 Post-LN 的区别是什么？为什么现代大模型（GPT、LLaMA）倾向于使用 Pre-LN？

**回答要点**：Post-LN 将 LayerNorm 放在残差连接之后（子层输出 LN 残差相加），Pre-LN 将 LN 放在子层之前（输入 LN 子层 残差相加）；Post-LN 在训练初期梯度不稳定，容易产生梯度爆炸需要 warmup；Pre-LN 的梯度流更稳定，训练更加平滑，无需 warmup 即可训练深层网络，因此成为现代 LLM 的标准做法。

**Q3（深挖）**：为什么 NLP 任务中使用 LayerNorm 而不是 BatchNorm？

**回答要点**：NLP 中输入序列长度可变，BatchNorm 在 batch 维度统计均值和方差，不同样本填充到相同长度导致统计量包含大量填充值噪声；LayerNorm 在特征维度独立归一化，与 batch size 和序列长度无关，对可变长序列天然友好；CV 任务中特征图大小通常固定（如 224×224），BatchNorm 更适用。

**Q4（实战）**：在 Transformer 中如果去掉残差连接或 LayerNorm，训练会发生什么？

**回答要点**：去掉残差连接：深层梯度无法回流，12+ 层 Transformer 几乎无法收敛，梯度消失导致浅层权重几乎不更新；去掉 LayerNorm：参数分布偏移累积，激活值不断增大，训练过程发散导致 NaN；两者同时存在才能保证深度 Transformer 的稳定训练。
