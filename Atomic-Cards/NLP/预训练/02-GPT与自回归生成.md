...
自回归生成的过程相当于一个**逐词续写器**：给定已写好的开头，预测下一个最合适的词，然后将这个词加入已写内容继续预测，直到生成完整的文本。

每一步的预测 $P(y_t \mid y_{<t})$ 是在整个词表上的概率分布。GPT 通过掩码自注意力确保在预测 $y_t$ 时只能看到 $y_1, y_2, \dots, y_{t-1}$，不能"偷看"未来的词。这种从左到右的单向约束是生成任务的核心——生成必须是因果的，不能依赖尚未生成的内容。

### 单向 vs 双向

GPT 的单向架构与 BERT 的双向架构对应两种不同的 NLP 任务范式：

- **理解任务**（分类、NER、QA）：需要完整上下文 → 双向编码器（BERT）
- **生成任务**（写作、翻译、对话）：需要逐词产生 → 单向解码器（GPT）

GPT 系列的核心贡献在于证明：**随着模型和数据规模的扩大，单向自回归模型在理解任务上也表现出色**，甚至超越了双向模型——涌现出的上下文学习、思维链等能力完全改变了 NLP 的范式。
