...
1. 将每个词表示为字符 n-gram 向量的和，而非整个词一个向量
2. OOV 词的子词片段可与训练语料中的已知子词重叠，从而组合出合理向量
3. 字符 n-gram 信息在罕见词上也有明显提升

**Q2（深挖）**：FastText 相对于 Word2Vec 在参数量、训练速度和内存占用上的代价是什么？这种权衡值不值？
**回答要点**：

1. 需要额外存储所有字符 n-gram 的向量，参数量显著增加
2. 训练速度慢于 Word2Vec
3. 对于形态丰富的语言或含大量 OOV/罕见词的场景，收益远大于代价

**Q3（实战）**：在一个领域专有术语极多的文本分类项目中，你会选择 FastText 还是 BERT？说说你的考量。
**回答要点**：

1. FastText 训练极快，适合做基线快速验证
2. BERT 精度更高但需要 GPU；工业级实践常先用 FastText 建立基线，再用知识蒸馏或小模型替代
3. FastText 的可解释性也更好（可直接查看词向量）

**Q4（边界）**：FastText 的子词策略在什么情况下反而会损害性能？举例说明。
**回答要点**：

1. 对于形态简单的语言（如中文），字符 n-gram 的信息增益有限
2. 当 n-gram 长度范围选得不好时可能引入噪声
3. 语料覆盖已很完整时子词造成不必要的存储开销；部分语言中超长的 n-gram 组合量爆炸

## 参考引用
- 需要理解 BERT与MLM预训练的相关知识，参见 [BERT与MLM预训练](../预训练/01-BERT与MLM预训练.md)
- 需要理解词嵌入与分布式表示的相关知识，参见 [词嵌入与分布式表示](03-词嵌入与分布式表示.md)
- 需要理解分词算法的相关知识，参见 [分词算法](02-分词算法.md)
