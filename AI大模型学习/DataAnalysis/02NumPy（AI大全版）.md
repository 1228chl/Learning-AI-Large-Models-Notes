**上一级：** [01NumPy模块（课堂版）](01NumPy模块（课堂版）.md)

**下一级：** [03Pandas模块（课堂版）](03Pandas模块（课堂版）.md)
...
| `string_` / `bytes_`                  | 固定长度字节串    | `S10` 表示 10 字节 |
...
- `take`：沿轴取指定索引的元素（类似花式索引但可指定 axis）
- `put`：将值放入指定位置
...
| 指数对数 | `exp`, `exp 2`, `expm 1`, `log`, `log 2`, `log 10`, `log 1 p`    |
...
- **C 顺序（行优先）**：`arr.flags['C_CONTIGUOUS']`
- **F 顺序（列优先）**：`arr.flags['F_CONTIGUOUS']`
