...
本笔记整合了神经网络的核心知识体系，系统讲解神经网络的基本概念、激活函数、参数初始化、损失函数、优化方法、正则化策略以及一个完整的分类案例。内容涵盖原理、数学公式、代码示例和调优建议，延续之前 PyTorch 笔记的详细风格。
...
- **输入层（Input Layer）**：接收原始数据特征，每个特征对应一个神经元。输入层不进行任何计算，仅传递数据。
- **隐藏层（Hidden Layers）**：位于输入层和输出层之间，可以有多个。隐藏层的神经元对输入进行加权求和并应用激活函数。网络的“深度”通常指隐藏层的数量。
- **输出层（Output Layer）**：产生最终的预测结果。对于回归任务，输出层通常没有激活函数（或使用恒等激活）；对于二分类，输出层常用 Sigmoid；对于多分类，输出层常用 Softmax。

**全连接（Fully Connected）** 的含义：第 $N$ 层的每个神经元与第 $N-1$ 层的所有神经元相连。同一层内的神经元之间没有连接。

**全连接神经网络的特点**：

- 数据以二维形式（样本数 × 特征数）在各层之间传递。
- 每个连接都有一个权重 $w$ 和偏置 $b$。
...
通过控制每个神经元的内部状态值、激活值的大小，以及每一层内部状态值的方差、激活值的方差，可以使整个神经网络训练得更稳定、更高效。
...
- **反向传播（BackPropagation，BP）**：根据损失函数计算输出层的误差，然后利用链式法则逐层向后传播梯度，计算每个权重和偏置对损失的贡献，用于参数更新。PyTorch 的 `autograd` 模块自动完成反向传播，无需手动推导。

**反向传播的链式法则示例**（以简单网络为例）：

假设输出层误差 $E$ 对权重 $w_5$ 的导数可分解为：

$$
\frac{\partial E}{\partial w_5} = \frac{\partial E}{\partial out_{o 1}} \cdot \frac{\partial out_{o 1}}{\partial net_{o 1}} \cdot \frac{\partial net_{o 1}}{\partial w_5}
$$

其中 $out_{o 1}$ 为输出层激活值，$net_{o 1}$ 为输出层内部状态值。详细推导过程详见优化方法部分。
...
- 光滑且连续，处处可导。
- 以 `(0, 0.5)` 为中心对称。
- 当输入值在 `[-6,6]` 之外时，输出值几乎为 0 或 1，信息丢失严重；只有在 `[-3,3]` 区间内才有较好的区分度。
...
- **梯度饱和**：当输入绝对值很大时，梯度接近于 0，导致参数更新极慢（梯度消失）。导数最大值为 0.25，在 5 层以内就可能产生梯度消失。
...
- 当输入 $x \leq -3$ 或 $x \geq 3$ 时，输出接近 -1 或 1，导数接近 0。
...
**适用场景**：隐藏层（但通常不如 ReLU 常用），尤其是需要零中心输出的场合。也可在隐藏层使用 Tanh，输出层使用 Sigmoid。
...
- **计算简单**：只需比较和取最大值，没有指数运算，计算量远小于 Sigmoid。
- **缓解梯度消失**：正半轴梯度恒为 1，不会出现梯度衰减。
- **稀疏激活**：负半轴输出为 0，使部分神经元不激活，增加模型稀疏性，减少参数依赖，缓解过拟合。
- 实际训练中收敛速度快。
...
从指定区间 `[a, b]` 的均匀分布中随机采样。默认区间为 `(0, 1)`，但通常需要根据输入维度调整范围，例如 $(-\frac{1}{\sqrt{d}}, \frac{1}{\sqrt{d}})$，其中 $d$ 为输入神经元个数。
...
**适用场景**：浅层网络或低复杂度模型。常见变体包含在 Xavier 和 Kaiming 初始化中。
...
**缺点**：随机选择范围不当可能导致梯度问题；对于深层网络，固定标准差可能导致梯度消失或爆炸。

**适用场景**：浅层网络（1-3 层隐藏层，总层数不超过 5 层）。
...
- `fan_in`：该层输入神经元的个数（当前层接收的上一层的神经元数量）
- `fan_out`：该层输出神经元的个数（当前层传递给下一层的神经元数量）
...
**优点**：适合 ReLU，能保持梯度稳定，防止 Dead ReLU 的恶化。

**缺点**：对非 ReLU 激活函数效果一般。

**适用场景**：深度网络（10 层及以上），使用 ReLU、LeakyReLU、PReLU 激活函数。
...
- 浅层网络（< 5 层）：随机初始化（如正态分布 std=0.01）可能足够。
- 深层网络（≥ 10 层）：必须使用 Xavier 或 Kaiming 来平衡方差。
...
**从概率角度理解**：最小化正确类别的预测概率的对数的负值，即最大化正确类别的对数概率。
...
| 概念 | 英文 | 含义 |
|------|------|------|
| **Epoch** | Epoch | 使用全部数据对模型进行一次完整训练 |
| **Batch** | Batch | 每次训练使用的小部分样本数量 |
| **Iteration** | Iteration | 使用一个 Batch 数据对模型进行一次参数更新 |

**示例**：假设数据集有 50000 个训练样本，Batch Size = 256，则：

- 每个 Epoch 要训练的图片数量：50000
- 每个 Epoch 具有的 Batch 个数：50000 / 256 ≈ 196（向上取整）
- 每个 Epoch 具有的 Iteration 个数：196
- 10 个 Epoch 具有的 Iteration 个数：1960

**梯度下降方式的区别**（以 Batch Size 大小区分）：

| 方式 | Batch Size | 特点 |
|------|------------|------|
| 批量梯度下降（BGD） | 全部样本 | 准确但慢，易陷入局部极小 |
| 随机梯度下降（SGD） | 1 | 波动大，收敛不稳定 |
| 小批量梯度下降（Mini-batch SGD） | 适中（如 32、64、128） | 折中方案，最常用 |

---

### 三、指数加权平均（Exponential Weighted Moving Average, EWMA）
...
其中 $g_t$ 为当前 mini-batch 的梯度，$\eta$ 为学习率。
...
### 六、学习率衰减（Learning Rate Scheduling）
...
学习率的选择至关重要：过小导致收敛慢，过大可能导致震荡甚至不收敛。

**代码演示**（观察不同学习率对简单函数 $y = 4 x^2$ 的优化过程）：
...
lr = 0.1  # 尝试不同学习率：0.01, 0.1, 0.125, 0.2, 0.3...
for i in range(10):
    y = func(x)
    y.backward()
    print(f"iter {i}: x={x.item():.4f}, grad={x.grad.item():.4f}")
    x.data.sub_(lr * x.grad)
    x.grad.zero_()
```

- 学习率 0.01：收敛慢，需要更多迭代。
- 学习率 0.125：一步到位到达最优解（x=0）。
- 学习率 0.2：跨过最优点，开始震荡。
- 学习率 0.3：梯度爆炸，数值溢出。

---

### 八、完整训练循环模板（含优化器和学习率调度）
...
**缩放的必要性**：训练阶段经过 Dropout 后的期望输出为 $\mathbb{E}[x_{\text{dropout}}] = (1-p)x \times \frac{1}{1-p} = x$，与测试阶段一致，保证了训练和测试时的输出量纲相同。
...
- **适用位置**：通常放在全连接层之后、激活函数之前或之后。对于卷积层，有时也使用 `nn.Dropout2d`（随机丢弃整个通道）。
- **常用丢弃率**：
  - 输入层：0.2 左右
  - 隐藏层：0.3 ~ 0.5
  - 输出层：通常不使用 Dropout
- **深层网络**：越接近输出层的隐藏层，丢弃率可以适当降低。
- **模型复杂度与 Dropout**：对于较小的模型或较简单的任务，丢弃率可以选择 0.3 或更小；对于非常深的网络，较大的丢弃率（如 0.5 或 0.6）可能会有效防止过拟合。
...
### 四、批量归一化（Batch Normalization, BN）
...
**数据集描述**：数据共有 2000 条，特征数为 20，标签为 0 ~ 3 的整数。主要特征包括：

| 特征名 | 含义 | 特征名 | 含义 |
|--------|------|--------|------|
| battery_power | 电池容量（毫安时） | pc | 主摄像头像素（百万） |
| blue | 是否有蓝牙 | px_height | 像素分辨率高度 |
| clock_speed | 处理器时钟速度 | px_width | 像素分辨率宽度 |
| dual_sim | 是否支持双卡 | ram | 随机存取存储器（MB） |
| fc | 前置摄像头像素 | sc_h | 手机屏幕高度（cm） |
| four_g | 是否有 4 G | sc_w | 手机屏幕宽度（cm） |
| int_memory | 内存（GB） | talk_time | 通话时间（h） |
| m_dep | 手机深度（cm） | three_g | 是否有 3 G |
| mobile_wt | 手机重量 | touch_screen | 是否有触控屏 |
| n_cores | 处理器核心数 | wifi | 是否能连 WiFi |

**目标**：预测 `price_range`（价格区间 0、1、2、3），这是一个**多分类问题**。
...
def load_and_prepare_data(csv_path='data/手机价格预测.csv', test_size=0.2, random_state=88):
    ...
    input_dim = X.shape[1]          # 特征数量，此处为 20
    num_classes = len(np.unique(y)) # 类别数量，此处为 4
...
设计一个三层全连接网络，使用 **Batch Normalization**、**Dropout** 和 **ReLU** 激活函数。参数初始化采用 Kaiming 初始化（适合 ReLU）。
...
================================================================
       ...
       BatchNorm1d-2               [16, 128]             256
           Dropout-3               [16, 128]               0
            Linear-4               [16, 256]          33,024
       BatchNorm1d-5               [16, 256]             512
           Dropout-6               [16, 256]               0
            Linear-7                 [16, 4]           1,028
================================================================
Total params: 37,508
...
**预期输出**（基线模型，使用 SGD 和简单三层网络时准确率约为 0.6425）：

```python
Epoch   1 | Train Loss: 1.3872 | Valid Acc: 0.3450
Epoch  10 | Train Loss: 0.8523 | Valid Acc: 0.6125
...
Epoch  50 | Train Loss: 0.5234 | Valid Acc: 0.6825
最佳验证准确率: 0.6825
```
...
基线准确率约为 **0.6425**（使用 SGD 和简单三层网络）。下面给出系统的调优方向。
...
```python
# 优化后：使用 Adam 优化器，增加网络深度，学习率调低
class ImprovedPhonePriceModel(nn.Module):
...
# 使用 Adam 优化器，学习率 0.0001，权重衰减 1e-4
...
1. **神经网络概述与结构**：神经元、全连接网络、前向/反向传播、参数量计算。
2. **激活函数**：Sigmoid、Tanh、ReLU、Softmax 的原理、优缺点及选择指南。
3. **参数初始化**：Xavier、Kaiming 初始化方法及选择策略。
4. **损失函数**：分类（CrossEntropyLoss、BCEWithLogitsLoss）与回归（L 1、MSE、SmoothL 1）损失。
5. **优化方法**：SGD、Momentum、AdaGrad、RMSProp、Adam 及学习率衰减策略。
6. **正则化**：权重衰减、Dropout、Batch Normalization 的原理与使用建议。
7. **完整案例**：手机价格分类，涵盖数据预处理、模型构建、训练、评估与调优。
