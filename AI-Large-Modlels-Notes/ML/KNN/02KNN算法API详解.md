**上一级：** [01KNN算法简介](01KNN算法简介.md)

**下一级：** [03距离度量](03距离度量.md)
...
- `fit` 的 X 要求：形状 `(n_samples, n_features)`。当只有一个特征时，每个样本需写成 `[值]`，多个样本则为 `[[值 1],[值 2]]`。
...
| 单个样本，n 个特征 | ` [[x1,x2,...,xn]] ` (1,n) | `[x 1,x 2,...,xn]` (n,) |
| 多个样本，1 个特征 | `[[x 1],[x 2],...]` (m,1) | ` [[x1,x2,...]] ` 或 `[x 1,x 2,...]` |
